...
<script>
    ...
    var myChart = new Chart(ctx, {
        ...
        data: {
             ...
             labels: [
                 {!!'"'.\Morilog\Jalali\CalendarUtils::strftime('Y/m/d', strtotime("-7 day",time())).'"'!!}, {!!'"'.\Morilog\Jalali\CalendarUtils::strftime('Y/m/d', strtotime("-6 day",time())).'"'!!}, {!!'"'.\Morilog\Jalali\CalendarUtils::strftime('Y/m/d', strtotime("-5 day",time())).'"'!!}, {!!'"'.\Morilog\Jalali\CalendarUtils::strftime('Y/m/d', strtotime("-4 day",time())).'"'!!}, {!!'"'.\Morilog\Jalali\CalendarUtils::strftime('Y/m/d', strtotime("-3 day",time())).'"'!!},
                 {!!'"'.\Morilog\Jalali\CalendarUtils::strftime('Y/m/d', strtotime("-2 day",time())).'"'!!}, {!!'"'.\Morilog\Jalali\CalendarUtils::strftime('Y/m/d', strtotime("-1 day",time())).'"'!!}, {!!'"'.\Morilog\Jalali\CalendarUtils::strftime('Y/m/d', time()).'"'!!}
             ],
             datasets: [{
                 label: 'تعداد بازدید',
                 data: [
                    @for($i=7;$i>-1;$i--) @php $visits=0;$sessionArray=array(0,2511777); $newtime=date('dmy',strtotime("-".$i." days")); @endphp @foreach($tracker as $track) @php $dbtime=date("dmy", strtotime($track->created_at)); @endphp @if($dbtime==$newtime) @if(!in_array($track->session_id,$sessionArray))
                   @php $visits=$visits+1 ; array_push($sessionArray,$track->session_id); @endphp @endif @endif @endforeach {{$visits}}, @endfor
                 ],
                 borderColor: [
                     'purple'
                 ],
